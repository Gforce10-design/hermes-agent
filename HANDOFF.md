...
> 최종 갱신: 2026-05-01 01:22 KST
> 브랜치: dev
> 상태: Hermes ↔ OpenClaw bridge 복구 코드 구현 및 검증 완료, 운영 gateway 재시작 전
...
## 다음에 할 일
1. Hermes/OpenClaw 변경을 각각 관련 파일만 stage/commit/push합니다.
2. 운영 반영 전 `hermes-gateway` 재시작 승인과 rollback 절차를 사용자에게 제시합니다.
3. 승인 후 Hermes gateway를 재시작하고 `hermes plugins list`, gateway logs, dry-run decision을 다시 확인합니다.
4. 실제 routing allow/deny 정책은 별도 승인 후 config에 적용합니다.